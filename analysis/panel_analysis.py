...
filtered_data = export_data[export_data[" Start"].dt.day == day_filter]
...
hist_values = np.histogram(export_data["Consumption (kWh)"], bins=30, range=(0, 3))
freqs = hist_values[0]
bins = hist_values[1]
st.bar_chart(freqs)
...
if export_checkbox:
    st.write("Export data", export_data)
